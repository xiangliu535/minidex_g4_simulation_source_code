...
HPGeParticleSourceMessenger::HPGeParticleSourceMessenger(HPGeParticleSource *pParticleSource):
	m_pParticleSource(pParticleSource), m_bShootIon(false)
{
	...
	m_pTypeCmd->SetGuidance("Either Point, Volume, CosmicNeutron");
	m_pTypeCmd->SetParameterName("DisType", true, true);
	m_pTypeCmd->SetDefaultValue("Point");
	m_pTypeCmd->SetCandidates("Point Volume CosmicNeutron");

        // use Fluak flag
        m_pFlukaUseCmd = new G4UIcmdWithABool("/xe/gun/useFluka", this);
        m_pFlukaUseCmd->SetGuidance("use Fluka or not");
        m_pFlukaUseCmd->SetGuidance("Default = false");

        // Fluka input file name
        m_pFlukaInputFileCmd = new G4UIcmdWithAString("/xe/gun/FlukaInputFileName", this);
        m_pFlukaInputFileCmd->SetGuidance("Set Fluka input file name");
        m_pFlukaInputFileCmd->SetParameterName("FlukaInputFile", true, true);
	...
	m_pAngTypeCmd->SetCandidates("iso direction CosmicNeutronDirection");
	...
	m_pEnergyTypeCmd->SetCandidates("Mono AmBe Flat100 Pb210Bremss CosmicNeutronEnergy");
...
HPGeParticleSourceMessenger::~HPGeParticleSourceMessenger()
{
	delete m_pTypeCmd;
        delete m_pFlukaUseCmd;
        delete m_pFlukaInputFileCmd;
	delete m_pShapeCmd;
	delete m_pCenterCmd;
	delete m_pHalfzCmd;
	delete m_pRadiusCmd;
...
void
HPGeParticleSourceMessenger::SetNewValue(G4UIcommand * command, G4String newValues)
{
	if(command == m_pTypeCmd)
		m_pParticleSource->SetPosDisType(newValues);

        else if (command == m_pFlukaUseCmd)
                m_pParticleSource->SetUseFlukaFlag(newValues);

        else if (command == m_pFlukaInputFileCmd)
                m_pParticleSource->ChangeFlukaInputFileName(newValues);
