  ...
  //--> check whether the input file is open
   ...
   //--> read in 
     ...
     for (G4int i=0;i<nEntries;i++) { 
      ...
      // only FLUKA z needs to be shifted by +7.5cm
      fFlukaInputFile >> fluka_code >> time_of_individual_particle  >> particle_position_x >> particle_position_y >> particle_position_z >> px >> py >> pz;
      particle_position_z = particle_position_z + 7.5;
	  
      // the Geant4 code is NOT the same as the PDG coding which we use for the simulation output !!!!
      Geant4_particleName = ConvertFlukaCodeToGeant4ParticleName(fluka_code);
